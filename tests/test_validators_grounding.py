...
# --------------------------------------------------------------------------
# VerbatimBelief / NormalizedBelief / FuzzyBelief — the same question at three strictnesses
# --------------------------------------------------------------------------
...
def test_verbatim_is_binary(value, expected):
    assert judge(VerbatimBelief("source_text"), value) == expected
...
def test_normalized_forgives_case_and_punctuation(value, expected):
    assert judge(NormalizedBelief("source_text"), value) == expected


def test_fuzzy_is_graded_and_deterministic():
    strong = judge(FuzzyBelief("source_text"), "ACME Oy")
    weak = judge(FuzzyBelief("source_text"), "ACME Oyj Corporation International")
    assert strong == 1.0
    assert 0.0 < weak < 1.0
    assert judge(FuzzyBelief("source_text"), "ACME Oyj Corporation International") == weak


def test_fuzzy_is_never_necessary_even_if_asked():
    """Never ``necessary`` -- the constructor enforces it rather than trusting."""
    assert FuzzyBelief("source_text").necessary is False
    assert FuzzyBelief("source_text", necessary=True).necessary is False


def test_grounding_says_nothing_without_a_source():
    empty = head("ACME Oy")
    assert VerbatimBelief("source_text")(empty, "vendor") is None
    assert FuzzyBelief("source_text")(empty, "vendor") is None


def test_grounding_says_nothing_without_a_candidate():
    e = FakeSnapshot(beliefs=[lambda e, a: None], source_text=SOURCE)
    assert VerbatimBelief("source_text")(e, "vendor") is None


# --------------------------------------------------------------------------
# TokenSubsetBelief — the classic hallucination tell
# --------------------------------------------------------------------------
...
def test_token_subset_over_numbers(value, expected):
    assert judge(TokenSubsetBelief("source_text"), value) == expected


def test_token_subset_is_graded_by_how_much_is_invented():
    p = judge(TokenSubsetBelief("source_text"), "999.00 plus 250.50 plus 7777.00")
    assert p == pytest.approx(2 / 3, abs=1e-4)   # rounded for a readable ledger


def test_token_subset_can_also_police_entities():
    belief = TokenSubsetBelief("source_text", kinds=("numbers", "entities"))
...
def test_token_subset_default_is_numbers_only():
    assert TokenSubsetBelief("source_text").kinds == ("numbers",)
    assert judge(TokenSubsetBelief("source_text"), "Globex Inc billed 999.00") == 1.0


# --------------------------------------------------------------------------
# QuoteIntegrityBelief / SpanValidBelief
# --------------------------------------------------------------------------
...
def test_quote_integrity(value, expected):
    assert judge(QuoteIntegrityBelief("source_text"), value) == expected
...
def test_quote_integrity_lets_the_unquoted_part_paraphrase_freely():
    ...
    assert judge(QuoteIntegrityBelief("source_text"), value) == 1.0


def test_span_valid():
    source = "ACME Oy — Helsinki"
    e = head({"start": 0, "end": 7, "text": "ACME Oy"}, source_text=source)
    assert ~SpanValidBelief("source_text")(e, "vendor") == 1.0

    wrong = head({"start": 0, "end": 4, "text": "ACME Oy"}, source_text=source)
    assert ~SpanValidBelief("source_text")(wrong, "vendor") == 0.0

    outside = head({"start": 0, "end": 999, "text": "ACME Oy"}, source_text=source)
    assert ~SpanValidBelief("source_text")(outside, "vendor") == 0.0


def test_span_valid_says_nothing_about_a_value_that_is_not_span_shaped():
    assert judge(SpanValidBelief("source_text"), "ACME Oy") is None


# --------------------------------------------------------------------------
# FrozenConsistentBelief — reads the ledger through the snapshot
# --------------------------------------------------------------------------
...
def test_frozen_consistent_compares_against_the_pinned_opinion():
    ...
    assert ~FrozenConsistentBelief()(agrees, "total") == 1.0
    assert ~FrozenConsistentBelief()(differs, "total") == 0.0


def test_frozen_consistent_is_silent_when_nothing_is_pinned():
    e = FakeSnapshot(entity="inv-1", ledger=Ledger(),
                     beliefs=[lambda e, a: (1249.5, 0.7)])
    assert FrozenConsistentBelief()(e, "total") is None
...
def test_frozen_consistent_uses_the_normalizing_comparator():
    ...
    assert ~FrozenConsistentBelief()(e, "vendor") == 1.0


# --------------------------------------------------------------------------
# NonEchoBelief — progress is enforced, not hoped for
# --------------------------------------------------------------------------
...
def test_non_echo_vetoes_a_verbatim_echo():
    ...
    got = NonEchoBelief()(e, "explanation")
...
def test_non_echo_passes_genuine_elaboration():
    ...
    assert ~NonEchoBelief()(e, "explanation") > 0.5
...
def test_non_echo_is_graded_not_binary():
    ...
    p = ~NonEchoBelief()(e, "explanation")
    assert 0.0 < p < 1.0


def test_non_echo_says_nothing_when_the_entity_carries_no_value():
    assert judge(NonEchoBelief(), "anything") is None


def test_non_echo_threshold_is_configurable():
    e = FakeSnapshot(value=CARRIED, beliefs=[lambda a, b: (CARRIED, 0.99)])
    assert NonEchoBelief(max_overlap=1.0).max_overlap == 1.0
    assert NonEchoBelief(max_overlap=1.0).id != NonEchoBelief().id
...
FAMILY_B = [VerbatimBelief("source_text"), NormalizedBelief("source_text"),
            TokenSubsetBelief("source_text"), QuoteIntegrityBelief("source_text"),
            SpanValidBelief("source_text"), FrozenConsistentBelief(), NonEchoBelief()]
...
@pytest.mark.parametrize("belief", FAMILY_B + [FuzzyBelief("source_text")],
                         ids=lambda b: b.id)
