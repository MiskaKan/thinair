...
def test_a_belief_is_a_pure_function_of_the_snapshot():
    ...
    from thinair.validators import Fuzzy

    e = head("ACME Oy Ltd", source_text="Bill from ACME Oy")
    first = Fuzzy("source_text")(e, "vendor")
    for _ in range(5):
        assert Fuzzy("source_text")(e, "vendor") == first
...
def test_id_derives_from_the_configuration():
    from thinair.validators import Range, Schema

    assert Range(0, 100).id == "range[0,100]"
    assert Schema(float).id == "schema[float]"
    assert Schema([{"desc": str, "amount": float}]).id == \
        "schema[[{amount:float,desc:str}]]"


def test_changing_the_configuration_mints_a_new_id():
    from thinair.validators import Range

    assert Range(0, 100).id != Range(0, 200).id


def test_same_configuration_is_the_same_belief():
    from thinair.validators import Range

    assert Range(0, 100).id == Range(0, 100).id
    assert lookup("range[0,100]").id == "range[0,100]"
...
def test_the_registry_does_not_intern_away_an_attachment_site_override():
    ...
    from thinair.validators import Verbatim

    strict = Verbatim("src")
    lenient = Verbatim("src", necessary=False)
...
def test_a_long_configuration_hashes_rather_than_sprawls():
    from thinair.validators import Enum

    belief = Enum([f"member-{i:03d}" for i in range(40)])
    assert belief.id.startswith("enum[#") and len(belief.id) < 40
...
def test_necessary_is_overridable_at_construction():
    ...
    assert Verbatim("source_text").necessary is True
    assert Verbatim("source_text", necessary=False).necessary is False
...
def test_options_do_not_leak_into_the_id():
    ...
    assert Verbatim("source_text").id == Verbatim("source_text", necessary=False).id
...
def test_generativity_is_observable_by_asking_an_empty_cell():
    ...
    assert not can_speak_into_an_empty_cell(Range(0, 10))
...
def test_consulting_the_panel_by_hand_writes_no_opinions():
    ...
    from thinair.validators import Range, TokenSubset

    ledger = Ledger()
    panel = [ScriptedBelief({"total": (1249.5, 0.93)}),
             TokenSubset("source_text"), Range(0, 1e6)]
...
def test_a_scripted_panel_writes_correct_opinions_into_a_ledger():
    ...
    panel = [proposer, TokenSubset("source_text"), Range(0, 1e6)]
    memo, active = {}, set()
    ...
    assert [o.belief for o in recorded] == \
        ["scripted:proposer", "tokenSubset[source_text,kinds=[numbers]]", "range[0,1000000]"]
